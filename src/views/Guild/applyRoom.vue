  ...
  <div>
    <el-card>
      ...
      >
        ...
        <el-table-column property="title" label="房间名称"> </el-table-column>
        <el-table-column property="roomnumber" label="申请房间号"> </el-table-column>
  <el-table-column property="uid" label="申请人id"> </el-table-column>
    <el-table-column property="unionid" label="申请工会id"> </el-table-column>
  ...
  setup() {
    ...
    const getlist = async () => {
      try {
        ...
        const res = await api.Guild.getRoomApplyList(option);
        const { data, code } = res;
        console.log("res", res);
        if (code == 0) {
    ...
    const refuseGuildTrue = async () => {
      try {
        ...
        const res = await api.Guild.RefuseRoomApply(option);
        const { code, message } = res;
        console.log("res", res);
        if (code == 0) {
          ElMessage({
    ...
    const adoptTrue = async () => {
      try {
        ...
        const res = await api.Guild.AdoptRoomApply(option);
        const { message, code } = res;
        if (code == 0) {
          getlist();
          ElMessage({
