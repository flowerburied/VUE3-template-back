<template>
  <div>
    <el-card>
      ...
      <el-descriptions title="充值" direction="vertical"  border>
        <el-descriptions-item  label="日">{{ tableData.day/100 }}</el-descriptions-item>
        <el-descriptions-item  label="周">{{
          tableData.week/100
        }}</el-descriptions-item>
        <el-descriptions-item  label="月">{{ tableData.month/100 }}</el-descriptions-item>
      </el-descriptions>

      <el-descriptions title="提现" direction="vertical"   border>
        <el-descriptions-item  label="日">{{
          SumWithdrawallist.day
        }}</el-descriptions-item>
        <el-descriptions-item  label="周" >{{
          SumWithdrawallist.week
        }}</el-descriptions-item>
        <el-descriptions-item  label="月">{{
          SumWithdrawallist.month
        }}</el-descriptions-item>
      </el-descriptions>
      <!-- <el-table
        border
  ...
  setup() {
    ...
    const getlist = async () => {
      try {
        ...
        // };
        const res = await api.amountMoney.SumRechargeLog();
        const { data, code } = res;
        console.log("res", res);
        if (code == 0) {
    ...
    const SumWithdrawal = async () => {
      try {
        const res = await api.amountMoney.SumWithdrawal();
        const { data, code } = res;
        console.log("res", res);
        if (code == 0) {
          datas.SumWithdrawallist = data;
        }
      } catch (err) {
        console.log("err!", err);
      }
